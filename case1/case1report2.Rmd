...
reduced <- lmer(log(blot) ~ dose1 + dose2 + (1|lab), data=dat, REML = FALSE)
summary(reduced) 
full <- lmer(log(blot) ~ dose1 + dose2 + age + (1|lab), data=dat, REML = FALSE)
summary(full)
anova(reduced, full)

#Age group (juvenile versus adult) affected log(blot) weight (chi2=2211.7, p<2.2e-16), lowering



m3 <- lmer(log(blot) ~ dose1 + dose2 + age + (1|lab), data=dat, REML = FALSE)
summary(m3) #variance due to lab heterogeneity dropped from .33 to .17

...
We then add a random effect on $\mu_i$, so $\text{Norm}(\mu, \psi^2)$ gives us a population distribution characterizing lab-to-lab variability. This allows our model to be generalizable to future labs. Furthermore, when calculating the MLE of each $\mu_i$, there is less variability because they are pulled in toward the grand mean $\mu$. This introduces bias, but the decrease in variance results in an overall benefit for the MSE.
...
m4 <- lmer(log(blot) ~ proto + log(body) + (I(1/(dose1+0.5)) + dose2 + 1|lab), data=dat_train, REML=FALSE)
...
compute_errs <- function(mod, test = dat_test){
  ...
  absolute <- abs_err(test$blot, preds)
  rootmean <- rms_err(test$blot, preds)
  return(list(abs_err = absolute, rms_err = rootmean))
}

random_dose_errs <- compute_errs(m2)
...
rownames(tab) <- c("Random Dose", "Fixed Dose")
kable(tab)
```
...
# sorry annie some of your variance numbers and stuff changed since I split the data up into a training and a testing set.
...
sim_params <- function(mod, nsim = 1000){
  ...
  randeffsd <- diag(summary(mod)[["varcor"]][["lab"]])

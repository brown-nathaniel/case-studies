...
plot(mod1,1, main = resid_title, xlab = "")
plot(mod1,2, main = qq_title, xlab = "")
...
plot_mvn <- function(mod = NA, lab_name = NA){
  ...
  plot(fitvals, resids, main=paste(resid_title, ": ", lab_name, " Lab", sep="")); abline(h=0)
  qqnorm(resids, main=paste(resid_title, ": ", lab_name, " Lab", sep="")); qqline(resids) 
  hist(resids, breaks=25, main=paste(resid_title, ": ", lab_name, " Lab", sep=""), col="gray")
}
...
In this random slope model, we see that the fit is comparable to that of the random intercept model. The AIC is 3285 (increased a bit from 3260), the variability due to lab is 0.039 (increased from 0.030), and the variability due to sources other than lab and protocol is 0.189 (decreased from 0.192). 

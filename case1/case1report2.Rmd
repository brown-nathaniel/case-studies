...
```{r}
lm1 <- lm(log(blot) ~ dose1 + dose2, data = dat)
# check for multicolinearity in dose1 and dose2
which(cor(dat[,4:5])>0.8)
...
plot(log(blot) ~ dose1 + dose2, data = dat)
# 2. Equal Variance: Do points deviate from regression line equally? 
# 3. Independence: randomness in residual plot 
# 4. Normality: qq plot tails are weird
par(mfrow=c(2,2))
plot(reg2)
par(mfrow=c(1,1))
...
View(dat)
library(lme4)
m3 <- lmer(log(blot) ~ dose1 + dose2 + proto + (1|lab), data=dat, REML = FALSE)
summary(m3)
m4 <- lmer(log(blot) ~ dose1 + dose2 + (1|lab), data=dat, REML = FALSE)
summary(m4) # AIC score increased and variance in residuals increased (removed proto)
# AIC: 5470, sd: .6628

# allow slope to vary as well as intercept
m5 <- lmer(log(blot) ~ (dose1 + dose2 + proto + 1)|lab, data=dat, REML = FALSE)
summary(m5) # correlation between protocols and dose is high, so remove from model
m6 <- lmer(log(blot) ~ (dose1 + dose2 + 1)|lab, data=dat, REML = FALSE)
summary(m6) # AIC score increased, variance in residuals increased (removed proto)
#AIC: 5551, sd: .6631
```

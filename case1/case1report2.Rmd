...
dummy_labs = model.matrix(~ lab, data = dat)
dat_list$dummy_lab = dummy_labs
dat_list$L = ncol(dummy_labs)
dummy_protocols = model.matrix(~ proto, data = dat)
dat_list$dummy_proto = dummy_protocols
dat_list$P = ncol(dummy_protocols)
...
stan_model3 = strwrap("
  data {
    ...
    real dose2[N];
    real blot[N];
    int<lower=1> L; // number of labs
  }
  parameters {
    vector[L] lab_intercepts;
    vector[L] lab_slopes;
    real<lower=0> sigma;
  ...
  model {
    real mu;
    // priors
    lab_intercepts ~ normal(0, 100);
    lab_slopes ~ normal(0, 100);
    for (i in 1:N) {
      mu = 
    }
  }

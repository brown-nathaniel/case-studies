...
summary(m3)
# correlation between protocols and dose is high, so remove from model
m4 <- lmer(log(blot) ~ dose1 + dose2 + (1|lab), data=dat, REML = FALSE)
summary(m4)
# allow slope to vary as well as intercept
m5 <- lmer(log(blot) ~ (dose1 + dose2 + 1)|lab, data=dat, REML = FALSE)
summary(m5) # AIC score increased 
```

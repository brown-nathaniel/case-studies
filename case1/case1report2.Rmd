...
In our previous random intercept model, we account for baseline differences between labs, but we also assume that the effects of the categorical variable proto is the same for each lab. This is not a valid assumption to make, so we introduce a random slope model. Now, proto, dose1, and dose2 have varying slopes. 

In this random slope model, more variability is associated with lab to lab heterogeneity (1.72, up from .03). We also see that the variability due to sources other than the random effects is 0.186 (decreased from 0.192). 

```{r}
# RANDOM SLOPE MODEL
m2 <- lmer(log(blot) ~ (dose1 + dose2 + proto + 1|lab), data=dat, REML=FALSE)
summary(m2)
```
...
In our previous random intercept model, we account for baseline differences between labs, but we also assume that the effects of the categorical variable proto is the same for each lab. This is not a valid assumption to make, so we introduce a random slope model. Now, proto has a random slope. 

In this random slope model, we see that the fit is comparable to that of the random intercept model. However, we also see that the variability due to sources other than the random effects (lab, protocol) is 0.189 (decreased from 0.192). 

...
#this also transforms the original dataframe
kernel_reg <- function(dat_all = dat, predictor = "invdose1", numknots = NA, s = NA, showplots = FALSE){
  ...
  colnames(X) <- paste0("kern", 1:ncol(X))
  ...
  return(list(datkerns = X, mod = mod))
}

kernlist1 <- kernel_reg()
dat_kern <- cbind(dat, kernlist1[[1]])
dat_train_kern <- dat_kern[trainrows,]
dat_test_kern <- dat_kern[testrows,]
m1 <- kernlist1[[2]]
plot_lmer(m4, parmfrow=c(3,3))
compute_errs(m4, dat_test_kern)
```
...
plot_lmer <- function(mod, parmfrow = NA, untrans_y = exp){
  ...
  continuouseffects <- grep("(dose)|(body)|(kern)", colnames(coef(mod)[[1]]), value=TRUE)

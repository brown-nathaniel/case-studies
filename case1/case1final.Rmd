...
format_lmer_list <- function(L = NA){
  ...
  randmeansshort <- cbind(randmeansshort, randmeanslong[,ncol(randmeanslong) - (1:3)])
...
```{r fig5, fig.cap="Diagnostic Plots of Model with Transformed Dose 1 and Blotted Uterus Weight"}
...
Based on the residual plots, this mixed model best satisfies the assumptions of linear regression. However, we note that taking the square root of log(blot) improves the residual plot at the cost of slightly lower predictive accuracy.
...
```{r fig6, fig.height=51, fig.width=30, fig.cap="Dose 1 Response Curves for Each Lab"}

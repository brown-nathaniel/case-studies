...
### Introduction
  ...
  The purpose of this project is to analyze rat bioassays. is there variation in results between labs? there shouldn't be. Here are a few plots showing the four protocols and other important things.
...
```{r, echo=FALSE}
L <- format_lmer_list(lmer_list(m1))
kable(L[[1]])
kable(L[[2]])
kable(L[[3]], caption = "Summary Statistics of Initial Model")
```

The diagnostic plots to assess model fit are below:
...
fixed_dose_errs <- compute_errs(m1)

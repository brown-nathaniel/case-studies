...
lmer_list <- function(mod){
  ...
  randeffmeans <- ranef(m4)[["lab"]] %>% apply(2,mean)
...
simdose1curves <- function(mod, nsim = 1000, untrans_y = exp){
  ...
  dose2 <- 0 
  dose2trans <- eval(parse(text = grep("dose2", rownames(L$random), value=T))) #in case dose2 needs to be transformed
  dose2effect <- dose2trans*L$random[grep("dose2", rownames(L$random)), "mean"]
  ...
  response <- untrans_y(fixedintereffect + protoeffect + bodyeffect + dose2effect + labsims + dose1effect)
...
```{r fig3, echo=FALSE, fig.height=10, fig.width=15}
plot_lmer(m1, c(2,3))
```
...
```{r fig4, fig.height=10, fig.width=15, echo=FALSE}
...
kable(L3[[1]]); kable(L3[[2]]); kable(L3[[3]])

plot_lmer(m3, parmfrow = c(2,3), expsq)

tab <- t(data.frame(x=as.numeric(errs3),y=as.numeric(errs2)))
colnames(tab) <- c("MAE","RMSE")
rownames(tab) <- c("Transformed Response and Dose 1", "Transformed Dose 1")
kable(tab)

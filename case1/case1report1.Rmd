...
kable(dosage_groups)
ggplot(data = dat ,
       mapping=aes(x=as.factor(group), y=log(blot))) + geom_boxplot() + xlab("dosage group")
...
We then add a random effect on $\mu_i$, so $\text{Norm}(\mu, \psi^2)$ gives us a population distribution characterizing lab to lab variability. This allows our model to be generalizable to future labs. Furthermore, when calculating the MLE of each $\mu_i$, there is less variability because they are "shrunk" toward the grand mean $\mu$. This introduces bias, but the decrease in varaiance reults in an overall benefit for the MSE. 
